    ...
    <main class="main-content">
      ...
      <div class="meter-card">
        ...
        <div
          class="meter-track"
          :class="{
            'is-loading': loadingCumulative,
            'is-error': cumulativeError,
          }"
        >
          <div
            class="meter-fill"
            :style="{
              width:
                loadingCumulative || cumulativeError
                  ? '0%'
          ...
          <div
            ...
            :style="{
              left:
                loadingCumulative || cumulativeError
                  ? '0%'
                  : `calc(${getProgressPercentage}% - 10px)`,
            }"
            aria-hidden="true"
          ></div>
        ...
        <div class="meter-footer">
          <span v-if="getCurrentLevel >= 4" class="done"
            >🎉 모든 레벨 달성!</span
          >
        </div>
      </div>
      ...
      <div class="floating-btn-group">
        ...
        <button class="floating-btn" @click="openNewsletter">
...
const getProgressPercentage = computed(() => {
  ...
  if (cur >= 40000)
    return Math.min(100, Math.round(((cur - 40000) / 20000) * 100));
  if (cur >= 20000)
    return Math.min(100, Math.round(((cur - 20000) / 20000) * 100));
...
const fetchAvatarAndItemData = async () => {
  try {
    ...
    if (itemsResponse?.data?.data) {
      const all = itemsResponse.data.data.map((it) => ({
        ...it,
        wearing: false,
      }));
      if (avatar.value) {
...
const fetchBubbleText = async () => {
  try {
    ...
    const textValue =
      ...
      '오늘의 퀴즈를 풀어보세요.';
    bubbleText.value =
      typeof textValue === 'string' ? textValue : '오늘의 퀴즈를 풀어보세요.';
  } catch (err) {
...
async function fetchUnconfirmedResults() {
  try {
    ...
    hasUnconfirmedResults.value =
      res?.data?.status === 200 ? !!res.data.data : false;
  } catch (e) {
...
function goToChallengeResult() {
  router.push('/challenge?tab=completed');
}
...
.main-content::before {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(#b3e5fc 0% 70%, #82c35c 70% 100%);
  z-index: 0;
}
.main-content > * {
  position: relative;
...
.result-fab-bubble .bubble-title {
  font-weight: 800;
  color: #2563eb;
}
.result-fab-bubble .bubble-text {
  color: #475569;
}
...
.pixel-cloud {
  ...
  height: 8px;
  background: #fff;
  box-shadow: 8px 0 #fff, 16px 0 #fff, -8px 8px #fff, 0 8px #fff, 8px 8px #fff,
    16px 8px #fff, 24px 8px #fff, -8px 16px #fff, 0 16px #fff, 8px 16px #fff,
    16px 16px #fff, 0 24px #fff, 8px 24px #fff;
  transform: scale(4);
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --brand: #7da0ff;
    --brand-strong: #5c86ff;
    --brand-soft-solid: #0f172a;
    --brand-soft-solid-2: #111827;
    --track: #1f2937;
  }
}
...
.chip-seed {
  background: linear-gradient(180deg, #8fcfb0, #79dd82);
}
.chip-apprentice {
  background: linear-gradient(180deg, #9bb9ff, #7398ff);
}
.chip-mage {
  background: linear-gradient(180deg, #bfb2ee, #a08be0);
}
.chip-master {
  background: linear-gradient(180deg, #f3bf9f, #eea67d);
}
...
.meter-track.is-error {
  background: var(--track);
  border-color: rgba(239, 68, 68, 0.25);
}
.meter-track.is-loading .meter-fill {
